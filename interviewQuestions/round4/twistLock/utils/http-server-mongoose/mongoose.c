...
void mg_send(struct mg_connection *nc, const void *buf, int len) {
  ...
  if (nc->flags & MG_F_UDP) {
    nc->iface->vtable->udp_send(nc, buf, len);
	  ...
	  printf("mg_send:: len >= 0 - tid:%lu, nc:%p, buf:%p, len:%d" ,pthread_self(), nc, buf, len);
...
void mg_send_http_chunk(struct mg_connection *nc, const char *buf, size_t len) {
  ...
  printf("mg_send_http_chunk - tid:%lu, buf:%p, len:%lu" ,pthread_self(), buf, len);
...
void mg_printf_http_chunk(struct mg_connection *nc, const char *fmt, ...) {
  ...
  if (len >= 0) {
	  printf("mg_printf_http_chunk:: len >= 0 - tid:%lu, nc:%p" ,pthread_self(), nc);
	  mg_send_http_chunk(nc, buf, len);
  }

  /* LCOV_EXCL_START */
  if (buf != mem && buf != NULL) {
	  printf("mg_printf_http_chunk:: buf != mem && buf != NULL - about to MG_FREE buf, in the address of:%p", buf);
	  MG_FREE(buf);
  }
  /* LCOV_EXCL_STOP */
}

/*
* First interview quesiotn:
* -------------------------
* Implement a simple regex match finder.
* 1) Given alpha-bet for str is [a-z] only.
* 2) (*) - means 0 chars or more.
* 3) (?) - means 1 chars or more.
* 4) If a givne pattern ends with a character --> than str MUST end with it as well. 
* 5) A pattern can end with a "wild card" (? or *).
* 6) You can assume pattern and str contains only [a-z] and [? *]  
*
* NOTES:
* a) Initial code is the code providied below.
* b) For the first implementaiton there is no need for efficeny -- just get it work.
*/
...
bool RunFirstInterviewQuestion()
{
	...
	cout << "isMatch(abcd, ab?d) returns:" << (isMatch("abcd", "ab?d") == true) << endl;	// sould return true
